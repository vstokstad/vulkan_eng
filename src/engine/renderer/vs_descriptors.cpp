...
vs_descriptor_set_layout::vs_builder &
vs_descriptor_set_layout::vs_builder::addBinding(
...
std::unique_ptr<vs_descriptor_set_layout>
vs_descriptor_set_layout::vs_builder::build() const {
...
vs_descriptor_pool::vs_builder &
vs_descriptor_pool::vs_builder::addPoolSize(VkDescriptorType descriptorType,
                                            uint32_t count) {
...
vs_descriptor_pool::vs_builder &
vs_descriptor_pool::vs_builder::setMaxSets(uint32_t count) {
  max_sets_ = count;
  return *this;
}

std::unique_ptr<vs_descriptor_pool>
vs_descriptor_pool::vs_builder::build() const {
  return std::make_unique<vs_descriptor_pool>(device_, max_sets_, pool_flags_,
                                              pool_sizes_);
}
...
vs_descriptor_pool::vs_descriptor_pool(
    vs_device &device, uint32_t maxSets, VkDescriptorPoolCreateFlags poolFlags,
...
bool vs_descriptor_pool::allocateDescriptor(
  ...
  if (vkAllocateDescriptorSets(device_.device(), &allocInfo, &descriptor) !=
    ...
    throw std::runtime_error(
        "allocate descriptor sets failed in descriptor_pool");
  }
  return true;
}

void vs_descriptor_pool::freeDescriptors(
...
vs_descriptor_writer::vs_descriptor_writer(vs_descriptor_set_layout &setLayout,
                                           vs_descriptor_pool &pool)
    : set_layout_{setLayout}, pool_{pool} {}

vs_descriptor_writer &
vs_descriptor_writer::writeBuffer(uint32_t binding,
...
vs_descriptor_writer &
vs_descriptor_writer::writeImage(uint32_t binding,
...
void vs_descriptor_writer::overwrite(VkDescriptorSet &set) {
  ...
  vkUpdateDescriptorSets(pool_.device_.device(), writes_.size(), writes_.data(),
                         0, nullptr);
}
} // namespace vs
