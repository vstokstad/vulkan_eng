...
namespace vs {
vs_window::vs_window(const int w, const int h, std::string name)
...
bool vs_window::shouldClose() const { return glfwWindowShouldClose(window_); }

void vs_window::createWindowSurface(VkInstance instance,
...
void vs_window::frameBufferResizedCallback(GLFWwindow *window, int width,
                                           int height) {
  auto _window =
      reinterpret_cast<vs_window *>(glfwGetWindowUserPointer(window));
