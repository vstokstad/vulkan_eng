...
namespace vs {
vs_pipeline::vs_pipeline(vs_device &device, const std::string &vert_path,
...
void vs_pipeline::create_graphics_pipeline(
  ...
  VkResult result =
      vkCreateGraphicsPipelines(device_.device(), VK_NULL_HANDLE, 1,
...
void vs_pipeline::create_shader_module(const std::vector<char> &code,
                                       VkShaderModule *shader_module) {
  VkShaderModuleCreateInfo createInfo{};
  createInfo.sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO;
  createInfo.codeSize = code.size();
  // this works because the char array is a vector with a standard allocator
...
void vs_pipeline::defaultPipelineConfigInfo(pipeline_config_info &config_info,
  ...
  config_info.multisample_info.sampleShadingEnable =
      enable_sample_shading ? VK_TRUE
  ...
  config_info.dynamic_state_enables = {VK_DYNAMIC_STATE_VIEWPORT,
                                       VK_DYNAMIC_STATE_SCISSOR, VK_DYNAMIC_STATE_LINE_WIDTH}; //TODO dynamic culling, need to enable extended dynamic states in  ?
