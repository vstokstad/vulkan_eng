...
namespace vs {
struct physics_system_push_constants {
...
void vs_simple_physics_system::createPipelineLayout(
    VkDescriptorSetLayout global_set_layout) {
  VkPushConstantRange push_constant_range{};
  push_constant_range.stageFlags =
      VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT;
  push_constant_range.offset = 0;
  push_constant_range.size = sizeof(physics_system_push_constants);
...
void vs_simple_physics_system::createPipeline(VkRenderPass render_pass) {
  ...
  pipeline = std::make_unique<vs_pipeline>(
      device_, "shaders/point_light.vert.spv", "shaders/point_light.frag.spv",
      pipeline_config);
}
...
void vs_simple_physics_system::update(const frame_info &frame_info,
                                          global_ubo &ubo) {
  auto rotate_light =
      glm::rotate(glm::mat4(1.f), frame_info.frame_time * glm::pi<float>(),
                  {0.1f, -1.f, .1f});
  ...
  for (auto &kv : frame_info.lights) {
    ...
    // copy light to ubo
    ubo.point_lights[light_index].position =
        glm::vec4(obj.transform_comp.translation, 1.f);
    ubo.point_lights[light_index].color =
        glm::vec4(obj.color, obj.point_light_comp->light_intensity);
...
void vs_simple_physics_system::render(frame_info &frame_info) {
  ...
  for (auto &kv : frame_info.lights) {
    auto &obj = kv.second;
    physics_system_push_constants push{};
    ...
    vkCmdPushConstants(frame_info.command_buffer, pipeline_layout_,
                       ...
                       0, sizeof(physics_system_push_constants), &push);
