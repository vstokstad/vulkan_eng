...
namespace vs
{
	...
	VkResult vs_swap_chain::submitCommandBuffers(
	...
	void vs_swap_chain::createSwapChain()
	{
		...
		current_present_mode_ = presentMode;
		...
		const std::vector<VkPresentModeKHR>& availablePresentModes)
	{
		for (const auto& availablePresentMode : availablePresentModes)
		{
			...
			{
				// if (current_present_mode_ != availablePresentMode)
				// {
		...
		// {
		// 	current_present_mode_ = VK_PRESENT_MODE_FIFO_KHR;
		// 	std::cout << "Present mode: V-Sync" << std::endl;
		// }
		return VK_PRESENT_MODE_FIFO_KHR;
	}

	VkExtent2D vs_swap_chain::chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities)
	{
